...
对于当前位之前的所有数位 从后到前遍历 寻找尽量靠后的数位上的数字减一 
但需要保证该数位上的数字减一后不能影响当前位到最高位仍为单调递增序列
该数位前的数位的数字与该数位上的数字相等 减一后必然破坏了单调递增性
即需要改数位尽量靠后且该数位前的数位的数字不与该数位上的数字相等（这里由于之前已经是单调递增序列 因此可以修改）
